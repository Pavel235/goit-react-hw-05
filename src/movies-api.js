...
export async function getMovies(nameOfMovie) {
  const url = `${BASE_URL}/search/movie?query=${nameOfMovie}`;
  const options = {
    headers: {
      Authorization:
...
export async function fetchMovieCast(movieId) {
  ...
  try {
    ...
    console.error("Error fetching movie cast details by id: ", error);
    return [];
  }
}
...
export async function fetchMovieReviews(movieId) {
  ...
  try {
    ...
    console.error("Error fetching movie reviews by id: ", error);
    return [];
  }
}
